// frontend/src/modules/recurso_judicial/RecursoJudicial.tsx
...
import { useRecursoJudicialStore } from './recursoJudicialStore';
import QuesitosModal from '../../components/common/QuesitosModal';

const RecursoJudicial: React.FC = () => {
    // Local state
    ...
    const {
        isLoading,
        error,
        recursoJudicialResult,
        gerarRecursoJudicial,
        currentFileBeingProcessed,
        clearState,
    } = useRecursoJudicialStore();
    ...
    const handleGerarRecurso = () => {
        ...
        gerarRecursoJudicial(fileToProcess);
    };
        ...
        <Container maxWidth="md" sx={{ mt: 4, mb: 4, p: 4, borderRadius: '12px', overflow: 'hidden' }}>
            <Stack spacing={3}>
                    <Typography variant="h5" component="h2" gutterBottom align="center" sx={{ fontWeight: 600, color: 'text.primary' }}>
                        Envie a sentença e demais documentos pertinentes
                    </Typography>
                    ...
                    {!isLoading ? ( /* Inputs */
                        <Box sx={{ textAlign: 'center', p: 2, border: '2px dashed', borderColor: 'grey.400', borderRadius: '8px', bgcolor: 'grey.50' }}>
                            ...
                            <Input type="file" inputRef={fileInputRef} onChange={handleFileChange} inputProps={{ accept: '.pdf', multiple: true, 'data-testid': 'file-input-recurso-judicial' }} sx={{ display: 'none' }} />
                    ...
                    <Box sx={{ textAlign: 'center', mt: 2 }}> {/* Submit Button */}
                        <Button variant="contained" color="primary" onClick={handleGerarRecurso} disabled={isLoading || selectedFiles.length === 0} sx={{ minWidth: '180px', px: 3, py: 1.5, fontSize: '1rem' }}>
...
export default RecursoJudicial;