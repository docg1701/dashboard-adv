// berry/src/views/recurso-judicial/index.jsx
...
import { useRecursoJudicialStore } from 'stores/recursoJudicialStore';

const FRASES_DIVERTIDAS = [
  'Recorrendo em 3, 2, 1...',
  'Polindo os argumentos para a Turma Recursal...',
  'Encontrando a jurisprudência perfeita...',
  'Aguarde, a sua tese está sendo fortalecida...',
  'Transformando inconformidade em um recurso robusto...'
];

const RecursoJudicialPage = () => {
  // Local state
  ...
  const { isLoading, error, recursoJudicialResult, gerarRecurso, currentFileBeingProcessed, clearState } = useRecursoJudicialStore();

  // Effect to open modal when results are ready
  useEffect(() => {
    if (recursoJudicialResult && !isLoading) {
      setIsModalOpen(true);
    }
  }, [recursoJudicialResult, isLoading]);
  ...
  return (
    <MainCard title="Recurso Judicial">
      <Stack spacing={3}>
        <Typography variant="body1" gutterBottom align="center">
          Envie a sentença e demais documentos pertinentes para gerar o recurso.
        </Typography>
        ...
        <Box sx={{ textAlign: 'center', mt: 2 }}>
          ...
          >
            {isLoading ? <CircularProgress size={24} color="inherit" /> : 'Gerar Recurso'}
          </Button>
        </Box>
        ...
        <QuesitosModal open={isModalOpen} onClose={handleCloseModal} title="Recurso Gerado" content={recursoJudicialResult || ''} />
      </Stack>
    </MainCard>
  );
};

export default RecursoJudicialPage;